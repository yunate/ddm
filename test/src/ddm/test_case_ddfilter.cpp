...
TEST(test_ddfilter, create_cluser_with_filter)
{
    auto it = create_cluser_with_filter(Fun, FunFilter);
    int x = 0;
    it(x, 1);
}
...
TEST(test_ddfilter, create_cluser_with_filter2)
{
    ...
    auto it = create_cluser_with_filter(callable, filter);
    int x = 0;
    it(x, 1);
    // it(1, 1);
}
